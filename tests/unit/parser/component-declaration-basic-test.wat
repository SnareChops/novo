;; Component Declaration Parser Basic Test
;; Tests parsing of simple component declarations

(module $component_declaration_basic_test
  ;; Import memory
  ...
  ;; Test basic component declaration parsing
  (func $test_basic_component (export "test_basic_component") (result i32)
    ...
    ;; Test input: "component example { }"
    (local.set $input_start (i32.const 2048))
    (local.set $input_len (i32.const 20))
    ...
    (i32.store8 offset=11 (local.get $input_start) (i32.const 120)) ;; 'x'
    (i32.store8 offset=12 (local.get $input_start) (i32.const 97))  ;; 'a'
    (i32.store8 offset=13 (local.get $input_start) (i32.const 109)) ;; 'm'
    (i32.store8 offset=14 (local.get $input_start) (i32.const 112)) ;; 'p'
    (i32.store8 offset=15 (local.get $input_start) (i32.const 108)) ;; 'l'
    (i32.store8 offset=16 (local.get $input_start) (i32.const 101)) ;; 'e'
    (i32.store8 offset=17 (local.get $input_start) (i32.const 32))  ;; ' '
    (i32.store8 offset=18 (local.get $input_start) (i32.const 123)) ;; '{'
    (i32.store8 offset=19 (local.get $input_start) (i32.const 125)) ;; '}'
  ...
  (func $test_basic_interface (export "test_basic_interface") (result i32)
    (local $input_start i32)
    (local $input_len i32)
    (local $token_count i32)
    (local $ast_node i32)
    ...
    ;; Test input: "interface storage { }"
    (local.set $input_start (i32.const 2048))
    (local.set $input_len (i32.const 21))

    ;; Store test string
    (i32.store8 offset=0 (local.get $input_start) (i32.const 105))  ;; 'i'
    (i32.store8 offset=1 (local.get $input_start) (i32.const 110))  ;; 'n'
    (i32.store8 offset=2 (local.get $input_start) (i32.const 116))  ;; 't'
    (i32.store8 offset=3 (local.get $input_start) (i32.const 101))  ;; 'e'
    (i32.store8 offset=4 (local.get $input_start) (i32.const 114))  ;; 'r'
    (i32.store8 offset=5 (local.get $input_start) (i32.const 102))  ;; 'f'
    (i32.store8 offset=6 (local.get $input_start) (i32.const 97))   ;; 'a'
    (i32.store8 offset=7 (local.get $input_start) (i32.const 99))   ;; 'c'
    (i32.store8 offset=8 (local.get $input_start) (i32.const 101))  ;; 'e'
    (i32.store8 offset=9 (local.get $input_start) (i32.const 32))   ;; ' '
    (i32.store8 offset=10 (local.get $input_start) (i32.const 115)) ;; 's'
    (i32.store8 offset=11 (local.get $input_start) (i32.const 116)) ;; 't'
    (i32.store8 offset=12 (local.get $input_start) (i32.const 111)) ;; 'o'
    (i32.store8 offset=13 (local.get $input_start) (i32.const 114)) ;; 'r'
    (i32.store8 offset=14 (local.get $input_start) (i32.const 97))  ;; 'a'
    (i32.store8 offset=15 (local.get $input_start) (i32.const 103)) ;; 'g'
    (i32.store8 offset=16 (local.get $input_start) (i32.const 101)) ;; 'e'
    (i32.store8 offset=17 (local.get $input_start) (i32.const 32))  ;; ' '
    (i32.store8 offset=18 (local.get $input_start) (i32.const 123)) ;; '{'
    (i32.store8 offset=19 (local.get $input_start) (i32.const 32))  ;; ' '
    (i32.store8 offset=20 (local.get $input_start) (i32.const 125)) ;; '}'
    ...
    (call $parse_interface_declaration (i32.const 0))
    (local.set $next_pos)  ;; Second return value
    (local.set $ast_node)  ;; First return value
    ...
    ;; Verify node type
    (local.set $node_type (call $get_node_type (local.get $ast_node)))
    ;; Check for interface declaration node type
    (if (i32.ne (local.get $node_type) (global.get $DECL_INTERFACE))
      (then (return (i32.const 0))))
  ...
  (func $run_tests (export "run_tests") (result i32)
    ...
    (local.set $test1_result (call $test_basic_component))

    ;; Run basic interface test
    (local.set $test2_result (call $test_basic_interface))
