;; WIT Export Complex Test
;; Tests WIT generation from components with imports and exports
...
(module
  ...
  ;; Test: Generate WIT for a component with imports and exports
  (func $test_component_with_imports_exports (export "_start")
    ...
    (local.set $import_node
      (call $create_decl_import
        (i32.const 1020)    ;; module name pointer: "wasi:http"
        (i32.const 9)       ;; module name length
        (i32.const 1030)    ;; item name pointer: "incoming-handler"
        (i32.const 16)))    ;; item name length

    ;; Add import as child of component
    (if (local.get $import_node)
      (then (drop (call $add_child (local.get $component_node) (local.get $import_node)))))

    ;; Create an export node
    ;; Export: "process"
    (local.set $export_node
      (call $create_decl_export
        (i32.const 1050)    ;; name pointer: "process"
        (i32.const 7)       ;; name length
        (i32.const 0)       ;; no alias
        (i32.const 0)))     ;; no alias length

    ;; Add export as child of component
    (if (local.get $export_node)
      (then (drop (call $add_child (local.get $component_node) (local.get $export_node)))))
    ...
    ;; Test that output contains expected component declaration
    (if (i32.eqz (call $contains_string
                    (local.get $output_ptr)
                    (local.get $output_len)
                    (i32.const 2000)    ;; "component http-proxy"
                    (i32.const 20)))
      (then (call $proc_exit (i32.const 4))))  ;; Fail if doesn't contain component declaration

    ;; Test that output contains expected import
    (if (i32.eqz (call $contains_string
                    (local.get $output_ptr)
                    (local.get $output_len)
                    (i32.const 2021)    ;; "import wasi:http"
                    (i32.const 16)))
      (then (call $proc_exit (i32.const 5))))  ;; Fail if doesn't contain import

    ;; Test that output contains expected export
    (if (i32.eqz (call $contains_string
                    (local.get $output_ptr)
                    (local.get $output_len)
                    (i32.const 2038)    ;; "export process"
                    (i32.const 14)))
      (then (call $proc_exit (i32.const 6))))  ;; Fail if doesn't contain export
  ...
  (data (i32.const 1000) "http-proxy")              ;; Component name
  (data (i32.const 1020) "wasi:http")               ;; Import module
  (data (i32.const 1030) "incoming-handler")       ;; Import item
  (data (i32.const 1050) "process")                ;; Export name
  (data (i32.const 2000) "component http-proxy")   ;; Expected component declaration
  (data (i32.const 2021) "import wasi:http")       ;; Expected import
  (data (i32.const 2038) "export process")         ;; Expected export
)
